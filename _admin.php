  <div class="breadcrum">
   <h3 class="breadcrum-title">Admin</h3>
  </div>

  <!-- dash-items -->
  <section class="section overview grid">
   <div class="overview__card">
    <div class="overview__card-details">
     <h1 class="details-heading">Yashwant Mangate</h1>
     <a class="details-mail" href="mailto: yash.mangate1@gmail.com">yash.mangate1@gmail.com</a> <br>
     <a class="details-contact" href="tel: 8329659032">8329659032</a>
    </div>

    <div class="admin_actions">
     <a href="#" class="admin_action-div">
      <i class='bx bx-edit-alt'></i>
     </a>
     <a href="#" class="admin_action-div">
      <i class='bx bx-trash-alt'></i>
     </a>
    </div>
   </div>
   <div class="overview__card">
    <div class="overview__card-details">
     <h1 class="details-heading">Kishan Aher</h1>
     <a class="details-mail" href="mailto: kishanaher12@gmail.com">kishanaher12@gmail.com</a> <br>
     <a class="details-contact" href="tel: 9112962305">9112962305</a>
    </div>

    <div class="admin_actions">
     <a href="#" class="admin_action-div">
      <i class='bx bx-edit-alt'></i>
     </a>
     <a href="#" class="admin_action-div">
      <i class='bx bx-trash-alt'></i>
     </a>
    </div>
   </div>
  </section>